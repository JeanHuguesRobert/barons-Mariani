---
title: "Sénatoriales 2026 — kit presse post-scrutin du 27 septembre"
subtitle: "Gabarit pré-armé à activer après proclamation et matérialisation des actes"
author: "Jean Hugues Noël Robert, baron Mariani"
date: "2026-09-26"
status: "prepared — not yet activated"
language: "fr"
license: "CC BY-SA 4.0"
visibility: "public"
document_role: "derived"
document_kind: "press-kit"
lifecycle_state: "prepared"
update_policy: "UP-DEFAULT-REVIEWED"
source_documents:
  - "../dossier_presse_2026-09.md"
  - "../investigation/chronology.md"
  - "../investigation/knowledge-matrix.md"
  - "../../autonomia/amendement_effectivite_article_72-5.md"
  - "../../contribution_commission_lois_autonomie_capacite.md"
review:
  status: "unreviewed"
  reviewed_by: []
---

# Kit presse post-scrutin — 27 septembre 2026

## Statut

Ce document est préparé le **26 septembre 2026**, veille du scrutin.

Il contient des blocs destinés à être complétés **uniquement après** les événements correspondants.

> **Aucun placeholder ci-dessous ne constitue un fait avant sa substitution par une trace vérifiée.**

## 1. Les six faits à publier en premier

À compléter après proclamation :

| Champ | Valeur |
|---|---|
| Élu proclamé en Haute-Corse | `[NOM]` |
| Résultat | `[VOIX / EXPRIMÉS / INSCRITS si disponibles]` |
| Heure / source de proclamation | `[HEURE + SOURCE]` |
| Interventions parlementaires post-scrutin | `[NOMBRE + DESTINATAIRES + HEURES]` |
| Requête au Conseil constitutionnel | `[CANAL + HEURE DE MATÉRIALISATION + HEURE DE DÉPÔT/TRANSMISSION]` |
| Version publique de la requête | `[LIEN PERMANENT + COMMIT + EMPREINTE]` |

## 2. Commentaire très court — gabarit

> **Le scrutin est terminé. Le dossier public permet désormais de comparer ce qui était juridiquement possible avec ce qui a été effectivement accessible. Le cas documenté ici montre le type d’écart qui peut subsister entre une possibilité juridique et une capacité réelle. L’amendement Baron Mariani propose que l’article 72-5 traite précisément les deux dimensions de cette effectivité : les effets des normes et l’accès aux habilitations nécessaires pour les produire.**

Ce texte doit être adapté aux faits observés et ne doit pas être utilisé pour attribuer une intention aux administrations ou juridictions.

## 3. Version journaliste — 30 secondes

> **Jean Hugues Noël Robert, qui se présente publiquement et électoralement sous le nom Baron Mariani, avait déposé le 11 septembre une déclaration de candidature aux sénatoriales de Haute-Corse. Son enregistrement a été refusé par le Tribunal administratif de Bastia le 14 septembre. Après le scrutin du 27 septembre, il [A FORMULER SELON ACTE RÉALISÉ] et publie le dossier documentaire correspondant. Il relie ce cas à une proposition constitutionnelle antérieure au contentieux : l’Autonomie de Capacité et l’amendement d’effectivité à l’article 72-5.**

## 4. Angle documentaire

La présentation recommandée distingue :

```text
fait électoral
≠
grief contentieux
≠
interprétation doctrinale
≠
proposition constitutionnelle
```

Le point historique recherché est le suivant :

> **la doctrine d’Autonomie de Capacité et la participation de C.O.R.S.I.C.A. au processus constitutionnel sont antérieures au contentieux sénatorial ; le cas électoral constitue un cas d’étude ultérieur de la différence entre possibilité formelle et capacité effective.**

## 5. Chronologie courte à fournir aux rédactions

- **1er octobre 2025** — annonce publique documentée de la candidature sénatoriale 2026 ;
- **20 mai 2026** — publication « Autonomia — 1974, 1991, 2026 — autonomie de capacité » ;
- **24 mai 2026** — courriel à un journaliste régional reliant explicitement candidature sénatoriale et réflexion sur l’autonomie ;
- **28 mai 2026** — contribution C.O.R.S.I.C.A. transmise à la commission des Lois ;
- **3 juin 2026** — C.O.R.S.I.C.A. mentionnée parmi les contributions écrites du rapport n° 2865 ;
- **11 septembre 2026** — déclaration de candidature déposée ;
- **14 septembre 2026** — refus d’enregistrement par le Tribunal administratif de Bastia ;
- **26 septembre 2026** — amendement d’effectivité v0.3-rc1 : ajout explicite de l’accès effectif aux habilitations nécessaires ;
- **27 septembre 2026** — `[RÉSULTAT + ACTES POST-SCRUTIN À INSÉRER]`.

## 6. Liens à envoyer aux journalistes

1. [Dossier presse principal](../dossier_presse_2026-09.md)
2. [Dossier canonique Sénatoriales 2026](../README.md)
3. [Chronologie probatoire](../investigation/chronology.md)
4. [Matrice des connaissances et inconnues](../investigation/knowledge-matrix.md)
5. [Amendement d’effectivité — « amendement Baron Mariani »](../../autonomia/amendement_effectivite_article_72-5.md)
6. [Contribution C.O.R.S.I.C.A. à la commission des Lois](../../contribution_commission_lois_autonomie_capacite.md)
7. `[REQUÊTE PUBLIQUE — À AJOUTER APRÈS MATÉRIALISATION]`
8. `[MANIFESTE DE DÉPÔT / ARCHIA — À AJOUTER APRÈS MATÉRIALISATION]`

## 7. Règles de publication dimanche

Avant toute diffusion, vérifier séparément :

- résultat réellement proclamé ;
- heure et source de la proclamation ;
- texte exact et destinataires des courriels réellement envoyés ;
- canal réellement utilisé pour la requête ;
- distinction entre heure de finalisation, heure de publication et heure de dépôt ;
- existence d’un accusé ou récépissé ;
- absence de données personnelles non nécessaires dans la version publique ;
- cohérence des liens et des identifiants de commit.

## 8. Traçabilité

La publication post-scrutin doit conserver au minimum :

```text
T1 = proclamation
T2 = version finale matérialisée
T3 = acte de saisine
T4 = accusé / réception institutionnelle
T5 = publication publique
```

Ces temps peuvent être différents. Ils ne doivent jamais être fusionnés artificiellement.

La cible symbolique de **12 h 00** ne modifie pas cette règle : si un acte est accompli à 12 h 17, il est enregistré à 12 h 17.

## 9. Message central

> **Ce qui peut être fait n’est pas nécessairement ce qui peut effectivement être fait.**

Le dossier post-scrutin doit permettre à chacun de vérifier les faits ayant conduit à cette proposition, sans transformer cette lecture doctrinale en fait juridictionnel.
