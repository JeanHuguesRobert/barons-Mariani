...
ai_assisted_by:
  - "GPT-5.6 Sol (drafting, packet closure integration and structuring)"
provenance:
  origin_type: "unknown"
...
## 14. Packet closure and the distributed continuation boundary

The parent architecture treats continuations as packetizable computational futures.

The classical continuation literature normally assumes a runtime-local universe (`call/cc`). A captured continuation can depend on memory, bindings, runtime conventions and code that remain reachable from the same execution environment:
...
### 14.1 Representation independence

Packet Closure does not require a native stack image. Equivalent representations include:
...
The invariant is **resumption closure**, not a single physical serialization.

---

## 15. Packet identity, historical truth, snapshots and capsules

The architecture distinguishes four objects that are easy to collapse accidentally:
...
The **Capsule** carries a *causal frontier* stating which portion of history its snapshot represents. A handler can then determine whether the Capsule is sufficient or whether additional state must be materialized before execution. This prevents the network packet from growing recursively with its entire history.

---

## 16. Logical stores, physical bindings and placements
...
- **Logical store**: the semantic role of the store;
- **Physical binding**: SQLite, PostgreSQL, Supabase, Git, object store, or archive technology;
- **Placement**: the fact that packet state currently exists at a specific binding.

The abstract machine reasons in terms of store capabilities (`ObjectStore`, `EventStore`, `SnapshotStore`, `SearchStore`, `IndexStore`, `ArchiveStore`, `TransactionalStore`, `SubscriptionStore`) with explicit latency, durability, sovereignty, cost and transactional semantics.

---

## 17. Governed external effects and the commit boundary

To prevent uncontrolled external side-effects under retries, branching and revocation, transitions materialize the explicit sequence:
...
This establishes an auditable commit boundary with explicit lifecycle states (`planned`, `prepared`, `authorized`, `committed`, `failed`, `compensated`, `observed`).

---

## 18. Four orthogonal state graphs

The distributed machine preserves four distinct graphs that intersect at the Cognitive Packet:
...
## 19. Revised abstract machine

The abstract state of the JHN machine is formalized as:

\[
\mathcal{J}^* = (P, C, A, S, K, M, G, B, T, X)
\]
...
executed through the cycle:
1. Select / attract packet \(p\);
2. Materialize closure if necessary;
3. Resolve admissible handler capability;
4. Execute locally or remotely;
5. Emit Artifacts \(O\), Events \(E\), downstream packets \(P'\);
6. Stage and commit governed effects \(X'\);
7. Update placements \(S'\), causal frontier \(C'\), knowledge \(K'\) and lineage \(T'\);
8. Return/assimilate when appropriate.

---

## 20. Agile validation path (Boundary crossings)

The architecture is validated through six successive boundary crossings:

- **Test A (Process boundary)**: Resume a closed packet after destroying all volatile handler state.
- **Test B (Store boundary)**: Move one packet from local SQLite to PostgreSQL/Supabase while preserving identity and closure.
- **Test C (Node boundary)**: Resume that packet on another node with no undocumented state from the first node.
- **Test D (Effect boundary)**: Execute one idempotent governed external effect through `EffectIntent` / `EffectReceipt`; prove retries and mandate changes are handled correctly.
- **Test E (Corpus boundary)**: Promote an operational packet to a Git/GitHub Corpus placement without changing packet identity.
- **Test F (Time boundary)**: Archive, remove hot copies, restore later, reconstruct authority and resume or inspect the packet.

---

## 21. Research questions before stronger claims
...
## 22. Initial falsifiable implementation target
