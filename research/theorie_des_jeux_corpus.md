---
title: "Théorie des jeux et corpus vivant"
subtitle: "Règles, triche, coopération, capture et Autonomie de Capacité"
author: "Jean Hugues Noël Robert"
affiliation: "Institut Mariani / C.O.R.S.I.C.A., 1 cours Paoli, F-20250 Corte, Corsica"
date: "2026-06-25"
status: "working_draft"
version: "0.1"
license: "CC BY-SA 4.0"
repository: "barons-Mariani"
path: "research/theorie_des_jeux_corpus.md"
document_role: "source"
document_kind: "research-paper"
visibility: "public"
lifecycle_state: "working"
human_validation_required: true
ai_assisted_by:
  - "ChatGPT — rédaction, structuration, explicitation autoporteuse"
review_expected_by:
  - "Grok / Claude / ChatGPT / humain — reviewer critique adverse, non décisionnel"
related_documents:
  - "research/noyau_doctrinal_rendre_capable.md"
  - "research/traceabilite_des_actes.md"
  - "research/review_protocol.md"
  - "research/methode_terrains_feconds.md"
  - "research/kudos.md"
  - "research/second_method.md"
related_projects:
  - "C.O.R.S.I.C.A."
  - "Institut Mariani"
  - "Cogentia"
  - "Kudocracy"
  - "Kudos"
  - "FractaVolta"
  - "Fractanet"
  - "Operium"
  - "Actarchia"
  - "Mare Nostrum"
  - "Autonomie de Capacité"
tags:
  - théorie des jeux
  - game theory
  - coopération
  - triche
  - capture
  - control plane
  - data plane
  - traçabilité
  - mandat
  - réputation
  - démocratie
  - autonomie de capacité
  - corpus vivant
classification_source: "cogentia.js"
classification_version: "1"
classification_rule: "research-paper"
classification_confidence: "medium"
---

# Théorie des jeux et corpus vivant

## Règles, triche, coopération, capture et Autonomie de Capacité

## Statut

Ce document est un **document source** du corpus. Il propose une lecture transversale de plusieurs projets, textes et intuitions déjà présents dans l’écosystème de Jean Hugues Noël Robert : C.O.R.S.I.C.A., Institut Mariani, Cogentia, Kudocracy, Kudos, FractaVolta, Fractanet, Operium, Actarchia, Mare Nostrum, #1755 et Autonomie de Capacité.

Il est conçu comme un document **autoporteur**. Cela signifie que ses thèses principales doivent pouvoir être comprises et critiquées sans lecture préalable des autres documents du corpus. Les notions propres au corpus sont donc introduites avant d’être utilisées.

Ce texte n’est pas un traité académique de théorie des jeux. Il utilise la théorie des jeux comme **grille de lecture** : un langage permettant de décrire les règles, les acteurs, les incitations, les asymétries d’information, les stratégies de coopération ou de défection, les captures du système et les conditions permettant de rendre la coopération rationnelle.

Formule directrice :

> Le corpus peut se lire comme une ingénierie des jeux sociaux : identifier les jeux où la triche devient rationnelle, rendre les coups visibles, réduire la capture du plan de contrôle, augmenter la capacité des joueurs légitimes et transformer progressivement les équilibres toxiques en équilibres coopératifs.

---

## 1. Résumé

La théorie des jeux étudie des situations où plusieurs acteurs prennent des décisions en tenant compte, explicitement ou implicitement, des décisions possibles des autres. Elle ne suppose pas nécessairement que les acteurs soient mauvais, cyniques ou uniquement égoïstes. Elle permet surtout de comprendre comment des règles, des informations et des incitations peuvent rendre rationnels des comportements qui détruisent pourtant le bien commun.

Ce document propose de relire le corpus à partir d’une hypothèse simple :

> Beaucoup de crises contemporaines ne viennent pas seulement d’un manque de vertu individuelle. Elles viennent de règles du jeu qui donnent une prime au vice et punissent la vertu.

Dans ce cadre, les projets du corpus ne sont pas dispersés. Ils explorent, chacun à sa couche, une même question :

> Comment concevoir des règles, des protocoles, des traces, des mandats, des réputations et des capacités qui rendent la coopération plus avantageuse que la triche, sans transformer la transparence en surveillance ni la coordination en capture ?

Cette lecture éclaire notamment :

- la **traçabilité des actes** : rendre les coups engageants visibles, imputables et contestables ;
- les **mandats explicites** : éviter que la délégation devienne un blanc-seing ;
- **Kudocracy** : penser la démocratie comme un jeu répété de délégation, de contrôle et de participation ;
- **Kudos** : rendre visibles les contributions, la reconnaissance et le contre-don sans créer une notation sociale opaque ;
- **FractaVolta / Fractanet** : construire des communs énergétiques, numériques et cognitifs où le protocole est commun mais les opérations restent diverses ;
- **Cogentia** : utiliser l’intelligence artificielle comme suggestion et éclairage, non comme prescription ou capture de la décision ;
- **Autonomie de Capacité** : ne pas seulement réclamer du pouvoir, mais créer des capacités réelles, vérifiables et anti-capture.

---

## 2. Vocabulaire minimal de théorie des jeux

Cette section introduit les notions générales nécessaires. Elle ne cherche pas l’exhaustivité.

### 2.1. Jeu

Un **jeu** est une situation structurée où plusieurs acteurs peuvent agir selon certaines règles, avec des conséquences qui dépendent aussi des actions des autres.

Un jeu peut être explicite, comme une partie d’échecs ou un match de football. Il peut aussi être implicite : une élection, un marché, une négociation, une copropriété, une administration, un réseau énergétique, une plateforme numérique ou une relation entre citoyens et mandataires.

### 2.2. Joueurs

Les **joueurs** sont les acteurs capables de poser des actes dans le jeu. Il peut s’agir de personnes physiques, d’élus, d’entreprises, d’administrations, d’associations, de plateformes, de collectifs, de systèmes automatisés ou d’agents IA utilisés par des humains.

Dans le corpus, un point important est que tous les joueurs ne doivent pas être traités de la même manière. Une personne vivante, une institution, une entreprise, une procédure automatisée et un agent conversationnel ne portent pas les mêmes conséquences, les mêmes droits ni les mêmes responsabilités.

### 2.3. Coup

Un **coup** est une action effectuée dans le jeu : voter, déléguer, publier, signer, financer, bloquer, tricher, révéler, corriger, auditer, sanctionner, se retirer, coopérer.

Dans le vocabulaire du corpus, un coup important devient souvent un **acte engageant** : un acte qui produit, prépare ou propage un effet significatif et doit donc pouvoir être relié à un mandat, une justification et une responsabilité.

### 2.4. Gain, coût et incitation

La théorie des jeux parle souvent de **payoff**, c’est-à-dire de gain ou de résultat pour un joueur. En français courant, on peut parler d’intérêt, d’avantage, de coût, de risque, de réputation, de pouvoir, de confort, d’argent, de sécurité ou de temps gagné.

L’idée centrale est simple :

> Les règles d’un système orientent les comportements en rendant certains coups avantageux et d’autres coûteux.

Si mentir rapporte plus que dire la vérité, si tricher rapporte plus que respecter la règle, si opacifier rapporte plus que rendre compte, alors le système produit mécaniquement une sélection adverse : les acteurs vertueux sont pénalisés et les acteurs toxiques sont avantagés.

### 2.5. Information

Un jeu peut être à information complète ou incomplète, symétrique ou asymétrique. Dans beaucoup de jeux sociaux réels, certains acteurs savent plus que d’autres, voient plus que d’autres, ou contrôlent mieux les canaux par lesquels l’information circule.

L’asymétrie d’information est l’un des grands leviers de la capture. Celui qui sait, mais n’est pas obligé de rendre compte, peut souvent orienter le jeu à son profit.

### 2.6. Équilibre

Un **équilibre** est une situation relativement stable où les joueurs n’ont pas intérêt, compte tenu des règles et des anticipations, à changer seuls de stratégie.

Un équilibre peut être mauvais. Une société peut rester coincée dans un équilibre d’opacité, de clientélisme, de défiance, d’abstention, de dépendance ou de sous-capacité parce que chaque acteur, pris isolément, trouve risqué de dévier.

L’enjeu du corpus n’est donc pas seulement de dénoncer un mauvais équilibre. Il est de créer des chemins praticables vers d’autres équilibres.

### 2.7. Jeux répétés

Un **jeu répété** est une situation où les acteurs interagissent plusieurs fois. Dans un jeu unique, trahir peut être avantageux si la sanction ne revient jamais. Dans un jeu répété, la mémoire, la réputation, la possibilité de réciprocité et la sanction future peuvent rendre la coopération plus rationnelle.

La démocratie, l’économie locale, les communs, la vie associative et les relations territoriales sont des jeux répétés. Le problème est que la répétition ne suffit pas. Encore faut-il une mémoire fiable, des règles claires et des sanctions proportionnées.

### 2.8. Coopération et défection

La **coopération** désigne les comportements qui rendent le jeu meilleur pour plusieurs acteurs ou pour le commun. La **défection** désigne les comportements qui exploitent le système à court terme au détriment des autres ou du commun.

La défection n’est pas toujours illégale. Elle peut prendre la forme de l’anti-jeu, du blocage, de la capture des règles, de l’usage stratégique du flou, de l’invisibilisation, de l’abus de procédure ou de la manipulation des canaux d’information.

---

## 3. Ce que la théorie des jeux n’est pas

Avant d’appliquer cette grille au corpus, il faut éviter trois contresens.

### 3.1. Ce n’est pas une réduction cynique de l’humain

Lire une situation comme un jeu ne signifie pas que les humains seraient uniquement calculateurs. Cela signifie que les situations sociales ont une structure. Même une personne morale peut être conduite à mal agir si le système rend coûteux le comportement juste et rentable le comportement toxique.

La théorie des jeux ne remplace donc pas l’éthique. Elle explique pourquoi l’éthique individuelle ne suffit pas lorsque les règles récompensent l’anti-jeu.

### 3.2. Ce n’est pas une justification de la manipulation

Comprendre les stratégies ne signifie pas qu’il faille manipuler. Au contraire, l’objectif du corpus est de rendre les stratégies visibles, discutables et gouvernables.

La ruse acceptable, dans cette perspective, est une ruse légale de reconfiguration : changer le terrain, révéler l’anti-jeu, créer des capacités, rendre la capture plus difficile. Elle n’est pas mensonge, tromperie ou prise de contrôle cachée.

### 3.3. Ce n’est pas une substitution de la technique à la politique

Les protocoles, registres, traces, agents IA, outils logiciels et architectures distribuées ne remplacent pas la décision humaine. Ils doivent aider les personnes à mieux décider, mieux contrôler, mieux transmettre et mieux corriger.

Dans la doctrine du corpus, les agents artificiels peuvent participer à la production de connaissance, mais les personnes vivantes seules gouvernent.

---

## 4. Présentation minimale du corpus concerné

Cette section introduit les principaux projets et notions internes utilisés dans la suite.

### 4.1. C.O.R.S.I.C.A. et Institut Mariani

**C.O.R.S.I.C.A.** est une association fondée à Corte. Elle porte une ambition non lucrative, open source et territoriale : explorer des formes nouvelles de démocratie, de transmission, d’autonomie, de mémoire et de capacité collective.

L’**Institut Mariani** est pensé comme l’émanation de recherche et développement de cette dynamique. Il sert à structurer des travaux doctrinaux, techniques et expérimentaux autour du corpus.

### 4.2. Corpus vivant

Le **corpus vivant** désigne l’ensemble versionné des textes, idées, projets, protocoles, arguments, objections, corrections, archives et produits déclinés. Il n’est pas seulement une bibliothèque. Il est un système de mémoire active, destiné à être repris, corrigé, transmis et augmenté.

La distinction importante est celle-ci :

- le **document source** conserve la formulation stable, traçable et révisable ;
- les **produits déclinés** adaptent ce document à un public ou à une scène : Substack, Facebook, discours, note politique, article académique, courrier, présentation.

### 4.3. Cogentia

**Cogentia** est la branche du corpus consacrée à la cognition outillée, aux agents IA, aux jumeaux numériques, aux paquets cognitifs, aux continuations et à la transmission. Son principe central est que l’IA doit augmenter la capacité humaine sans capturer la décision.

Dans le vocabulaire du corpus, l’IA doit produire des suggestions, des éclairages, des objections, des synthèses et des continuations. Elle ne doit pas prescrire le vote, se substituer au jugement humain ou devenir une autorité opaque.

### 4.4. Kudocracy

**Kudocracy** désigne une approche de la démocratie outillée fondée sur la participation, la délégation contrôlable, les mandats explicites, la traçabilité et la possibilité de reprendre ou rediriger son pouvoir d’agir.

Sous l’angle de la théorie des jeux, Kudocracy cherche à transformer la démocratie en jeu répété mieux instrumenté : les citoyens ne sont plus seulement appelés à voter ponctuellement, ils peuvent déléguer, contrôler, corriger, révoquer et participer selon leurs capacités.

### 4.5. Kudos

**Kudos** est une proposition de monnaie complémentaire ou couche de reconnaissance adossée à des transactions réelles. L’euro clôt la transaction ; le Kudos ouvre le contre-don.

Sous l’angle de la théorie des jeux, Kudos cherche à rendre visibles des contributions qui disparaissent dans le paiement marchand ordinaire : reconnaissance, confiance, participation territoriale, don, contre-don, mémoire publique volontaire.

### 4.6. FractaVolta et Fractanet

**FractaVolta** est la branche énergie / infrastructure du corpus. Elle cherche à packetiser l’énergie, c’est-à-dire à la rendre plus modulable, routable, traçable, partageable et gouvernable.

**Fractanet** désigne plus largement une architecture de réseau distribuée où des ressources, capacités ou flux peuvent être coordonnés sans être absorbés par un centre unique.

Sous l’angle de la théorie des jeux, FractaVolta et Fractanet posent la question des communs : comment coordonner des producteurs, consommateurs, stockeurs, opérateurs et territoires sans recréer une capture centrale ?

### 4.7. Operium et Actarchia

**Operium** peut être compris comme une approche de documentation et de pilotage opérationnel : comprendre l’état courant d’un système, ses évolutions envisagées et sa santé opérationnelle.

**Actarchia** désigne une logique de registre des actes : rendre les actes engageants visibles, imputables, vérifiables et contestables.

Sous l’angle de la théorie des jeux, ces deux branches réduisent l’opacité. Elles transforment des jeux à information faible ou manipulable en jeux où les coups importants laissent une trace exploitable.

### 4.8. Autonomie de Capacité

L’**Autonomie de Capacité** est une notion politique centrale du corpus. Elle s’oppose à une autonomie purement formelle ou institutionnelle. Elle ne demande pas seulement : « Qui a le pouvoir de décider ? » Elle demande : « Qui a réellement la capacité d’agir, de contrôler, de produire, de vérifier, de corriger et de transmettre ? »

Une autonomie de papier peut renforcer des élus ou institutions sans augmenter la capacité réelle des habitants. Une autonomie de capacité doit au contraire augmenter les moyens effectifs des personnes, des communes, des collectifs et des territoires.

### 4.9. Mare Nostrum et #1755

**Mare Nostrum** est une projection méditerranéenne articulant énergie décarbonée, inférences IA, souveraineté distribuée et coopération entre territoires, notamment insulaires.

**#1755** désigne une initiative de reconnaissance historique autour de la Constitution corse de 1755 et de la place de la Corse dans l’histoire mondiale de la souveraineté populaire, du constitutionnalisme et des révolutions modernes.

Sous l’angle de la théorie des jeux, #1755 est un jeu de reconnaissance : obtenir d’abord une reconnaissance limitée, rigoureuse et non conflictuelle, capable d’ouvrir ensuite un espace plus large de discussion et de transmission.

---

## 5. Thèse générale : changer les règles du jeu

La thèse générale de ce document peut se formuler ainsi :

> Le corpus ne cherche pas seulement à produire des idées, des logiciels ou des institutions. Il cherche à modifier les jeux sociaux dans lesquels les personnes, les institutions, les plateformes, les territoires et les agents techniques interagissent.

Un mauvais jeu social se reconnaît à plusieurs symptômes :

- la triche rapporte plus que le respect des règles ;
- l’opacité protège mieux que la transparence ;
- la loyauté de camp rapporte plus que la recherche du vrai ;
- la délégation devient un blanc-seing ;
- les mandataires contrôlent ceux qui devraient les contrôler ;
- les citoyens rationnels se retirent parce que participer semble coûteux et inefficace ;
- les acteurs honnêtes sont plus exposés que les acteurs cyniques ;
- les institutions empêchent l’expérimentation avant même de pouvoir juger ses résultats ;
- les plateformes capturent les données, les profils, les récits ou les canaux de visibilité.

Dans un tel contexte, l’appel moral à la vertu est insuffisant. Il faut changer les incitations, les règles, les traces, les points de contrôle et les possibilités d’action.

Formule de doctrine :

> Il ne suffit pas de demander aux joueurs d’être vertueux lorsque le jeu récompense l’anti-jeu. Il faut rendre la vertu avantageuse et la triche coûteuse.

---

## 6. Prime au vice et punition de la vertu

La formule « prime au vice et punition de la vertu » résume une situation où les règles apparentes ne produisent plus les comportements qu’elles prétendent encourager.

Un système peut afficher des valeurs élevées — justice, démocratie, service public, transparence, intérêt général — tout en récompensant concrètement :

- l’opacité ;
- le retard stratégique ;
- la complexité dissuasive ;
- l’entre-soi ;
- la connivence ;
- la captation des procédures ;
- la communication sans vérifiabilité ;
- la posture morale sans actes traçables.

Dans ce type de jeu, le citoyen ou l’acteur vertueux subit un double coût :

1. il respecte des règles qui limitent ses coups ;
2. il affronte des acteurs qui exploitent ces mêmes règles sans réciprocité.

Le résultat est une sélection adverse : les joueurs capables d’exploiter l’opacité montent dans le système, tandis que les joueurs attachés à la cohérence, à la preuve ou à la responsabilité sont marginalisés, épuisés ou ridiculisés.

La réponse du corpus n’est pas de supposer que tous les acteurs sont corrompus. Elle est de concevoir des architectures qui n’exigent pas une vertu héroïque permanente.

Formule :

> Un bon système ne suppose pas que tous les joueurs soient vertueux. Il limite les gains de la triche, augmente les gains de la coopération et rend les abus détectables avant qu’ils deviennent irréversibles.

---

## 7. Control plane et data plane

La distinction entre **control plane** et **data plane** vient des réseaux informatiques, mais elle peut servir de métaphore générale.

Le **data plane** est la couche où circulent les flux : énergie, argent, votes, données, services, décisions, transactions, publications.

Le **control plane** est la couche qui définit les règles de circulation : qui a le droit d’agir, selon quelles procédures, avec quelles autorisations, quelles limites, quelles sanctions, quelles priorités, quelles exceptions.

Dans beaucoup de conflits sociaux ou politiques, le problème n’est pas seulement qu’un joueur triche dans le data plane. Le problème plus profond est qu’il tente de capturer le control plane.

Exemples :

- contrôler l’ordre du jour plutôt que seulement voter ;
- contrôler les critères de recevabilité plutôt que seulement argumenter ;
- contrôler les canaux médiatiques plutôt que seulement répondre ;
- contrôler les données d’audit plutôt que seulement produire ;
- contrôler les définitions officielles plutôt que seulement débattre ;
- contrôler le registre des actes plutôt que seulement poser des actes.

Formule :

> Tricher dans le data plane est grave. Capturer le control plane est plus grave, car cela permet de redéfinir ce qui comptera ensuite comme règle, faute, preuve, exception ou victoire.

Cette distinction éclaire plusieurs branches du corpus :

- Kudocracy veut rendre le control plane démocratique plus contrôlable ;
- Fractanet veut éviter qu’un commun énergétique ou numérique soit capturé par un opérateur central ;
- Cogentia veut empêcher que les agents IA capturent le jugement humain ;
- Operium et Actarchia veulent documenter les actes qui modifient réellement l’état du système ;
- l’Autonomie de Capacité refuse qu’un transfert de pouvoir institutionnel renforce seulement un control plane local non contrôlé.

---

## 8. Football, échecs et pédagogie stratégique

La théorie des jeux peut sembler abstraite. Pourtant, ses intuitions sont déjà présentes dans des pratiques populaires ou largement comprises.

### 8.1. Football

Le football permet de comprendre :

- les règles communes ;
- le terrain ;
- l’arbitrage ;
- la faute tactique ;
- l’anti-jeu ;
- le hors-jeu ;
- le collectif ;
- la stratégie ;
- le rapport entre talent individuel et système de jeu ;
- la différence entre gagner loyalement, tricher dans le match et acheter le match.

L’expression « acheter le match » est particulièrement utile. Elle montre immédiatement la différence entre jouer mieux et capturer le jeu.

Dans une démocratie capturée, le problème n’est pas seulement qu’un camp joue mieux. Le problème est que certains acteurs peuvent influencer les règles, l’arbitrage, le calendrier, la visibilité, les sanctions et les récits d’après-match.

### 8.2. Échecs

Les échecs permettent de comprendre :

- la position ;
- le tempo ;
- le sacrifice ;
- le piège ;
- la menace ;
- la coordination des pièces ;
- les coups forcés ;
- le zugzwang, c’est-à-dire une situation où tout coup disponible aggrave la position ;
- la différence entre tactique courte et stratégie longue.

Beaucoup de situations politiques ou institutionnelles ressemblent à des positions fermées : le joueur faible ne peut pas gagner frontalement, mais il peut changer le rythme, ouvrir une colonne, forcer une clarification, rendre visible une contradiction ou transformer une défense adverse en preuve de capture.

### 8.3. Transfert pédagogique

Une hypothèse pédagogique importante pour la Corse est que les compétences stratégiques existent déjà dans la culture ordinaire : analyse du football, goût des échecs, sens du terrain, mémoire des rapports de force, prudence face aux appareils.

L’enjeu est de transférer ces compétences vers l’économie, l’énergie, la démocratie, l’autonomie et la production de capacité.

Formule :

> Beaucoup savent déjà analyser un match ou une position. Il faut apprendre à lire une institution, un marché, une plateforme ou une élection avec le même sens des règles, des coups, des incitations et de l’anti-jeu.

---

## 9. Jeux répétés, mémoire et réputation

La coopération durable suppose généralement trois conditions :

1. les acteurs savent qu’ils se reverront ou que leurs actes auront une mémoire ;
2. les comportements significatifs sont observables ou reconstructibles ;
3. les abus ont un coût futur.

Dans les petits groupes, cette mémoire peut être informelle. Dans les sociétés complexes, elle doit être outillée.

Mais une mémoire outillée peut dériver vers la surveillance. La distinction est donc essentielle :

- tracer les **actes engageants**, pas ficher toute la personne ;
- rendre les mandataires et institutions responsables, pas exposer inutilement les vies privées ;
- documenter les contributions volontaires, pas produire une note sociale obligatoire ;
- permettre la contestation, la correction et le droit à l’erreur.

Kudos, par exemple, doit être pensé comme une mémoire volontaire de reconnaissance et de contre-don, non comme un crédit social. La traçabilité des actes doit viser les actes ayant un effet sur autrui ou sur un mandat, non les micro-événements privés.

Formule :

> La mémoire utile n’est pas la surveillance totale. C’est la capacité de retrouver les actes qui engagent, les mandats qui les autorisent, les raisons qui les justifient et les mécanismes qui permettent de les contester ou de les corriger.

---

## 10. Dilemme du prisonnier et dilemme du citoyen

Le dilemme du prisonnier est un exemple classique : deux acteurs auraient collectivement intérêt à coopérer, mais chacun peut être tenté de trahir si la trahison semble plus rentable individuellement.

Transposé à la vie publique, on peut parler d’un **dilemme du citoyen**.

Un citoyen peut se dire :

- ma participation coûte du temps ;
- mon influence semble faible ;
- les élus décideront sans moi ;
- les appareils maîtrisent les procédures ;
- les médias dominants ne relaieront pas ;
- les autres citoyens ne participeront pas davantage ;
- je risque de m’exposer inutilement.

L’abstention, le retrait ou le cynisme deviennent alors rationnels individuellement, même s’ils aggravent collectivement la capture.

La réponse ne peut pas être seulement morale : « participez davantage ». Il faut réduire le coût de la participation et augmenter son efficacité réelle.

C’est ici que Kudocracy, Cogentia, la traçabilité des actes et l’Autonomie de Capacité convergent :

- rendre l’information compréhensible ;
- permettre des délégations contrôlables ;
- rendre les actes des mandataires visibles ;
- donner aux citoyens des outils d’analyse ;
- permettre des contributions graduées ;
- rendre la reprise de contrôle possible.

Formule :

> La démocratie capable doit sortir le citoyen du dilemme où se retirer est rationnel parce que participer semble inutile.

---

## 11. Passager clandestin et communs

Un **passager clandestin** est un acteur qui bénéficie d’un commun sans contribuer équitablement à son entretien.

Les communs énergétiques, numériques, cognitifs ou territoriaux sont vulnérables à ce problème. Si certains produisent, documentent, financent, maintiennent ou prennent des risques tandis que d’autres capturent les bénéfices sans contribuer, le commun s’épuise.

Mais la solution ne doit pas être une centralisation autoritaire. Elle doit permettre :

- de rendre les contributions visibles ;
- de distinguer usage légitime et extraction abusive ;
- de reconnaître les mainteneurs ;
- de répartir les coûts ;
- de protéger les ressources partagées ;
- d’éviter qu’un acteur dominant transforme le commun en dépendance.

FractaVolta et Fractanet relèvent directement de cette problématique. Un réseau d’énergie ou d’inférences peut produire de la capacité collective seulement si ses règles empêchent la capture par un opérateur, une plateforme, une institution ou un groupe d’intérêts.

Formule :

> Mettre en commun ce qui permet d’agir ; laisser divers ceux qui agissent ; rendre vérifiable ce qui a été fait.

---

## 12. Transparence, surveillance et asymétrie

La transparence est un levier central du corpus. Mais elle n’est légitime que si elle respecte une asymétrie démocratique : plus un acteur exerce un pouvoir sur autrui, plus ses actes engageants doivent être visibles et imputables.

La transparence exigée d’un élu, d’une institution, d’un mandataire, d’un opérateur de réseau ou d’un agent automatisé ne doit pas être confondue avec l’exposition permanente de la vie privée des personnes ordinaires.

Principe :

> Transparence renforcée pour les actes publics et les mandataires ; protection renforcée pour les personnes privées.

Cette distinction permet de répondre à une objection majeure : la traçabilité pourrait devenir surveillance. Oui, si elle est mal conçue. Non, si elle est bornée par des seuils, des finalités, des droits de contestation, des durées, des mandats et une séparation claire entre actes engageants et vie personnelle.

---

## 13. IA comme révélateur et stabilisateur

L’intelligence artificielle modifie les jeux sociaux parce qu’elle réduit fortement le coût de certaines opérations : lire, résumer, comparer, chercher des incohérences, produire des variantes, simuler des objections, surveiller des changements, documenter des décisions, préparer des courriers, relier des idées éloignées.

Dans le corpus, l’IA doit jouer deux rôles principaux.

### 13.1. Révélateur

L’IA peut rendre visibles :

- les contradictions ;
- les incohérences ;
- les promesses non tenues ;
- les asymétries d’information ;
- les captures de procédure ;
- les arguments faibles ;
- les dépendances cachées ;
- les effets de bord ignorés.

Elle peut donc modifier les jeux où l’opacité protège les acteurs dominants.

### 13.2. Stabilisateur

L’IA peut aussi produire des stabilisateurs :

- check-lists ;
- revues critiques ;
- synthèses ;
- protocoles ;
- registres ;
- classifications ;
- corrections ;
- suivis ;
- produits déclinés à partir d’un document source.

Elle aide alors à maintenir le corpus, les projets et les institutions dans un état plus cohérent.

### 13.3. Limite démocratique

L’IA ne doit pas devenir un joueur souverain du jeu démocratique. Elle ne vit pas sous les lois qu’elle peut influencer. Elle ne porte pas les conséquences politiques, existentielles ou matérielles des décisions collectives.

Formule :

> L’IA peut éclairer les coups possibles. Elle ne doit pas capturer le joueur.

---

## 14. Autonomie de Capacité : changer le jeu corse

L’autonomie institutionnelle peut être pensée comme un transfert de compétences. L’Autonomie de Capacité demande davantage : les habitants, communes, collectifs et acteurs locaux deviennent-ils réellement plus capables ?

Sous l’angle de la théorie des jeux, la question est :

> Le nouveau jeu rend-il les citoyens plus puissants face aux captures, ou rend-il seulement certains mandataires plus puissants face aux citoyens ?

Un transfert de pouvoir sans contrôle peut produire un mauvais équilibre : plus de pouvoir local, mais pas plus d’imputabilité ; plus de discours d’autonomie, mais pas plus de capacité habitante ; plus de control plane local, mais toujours peu de contrôle citoyen.

L’Autonomie de Capacité exige donc :

- des mandats explicites ;
- des actes traçables ;
- des expérimentations locales réelles ;
- des outils de contrôle citoyen ;
- une capacité énergétique, numérique, alimentaire et cognitive ;
- des communs protégés contre la capture ;
- une pédagogie des règles du jeu ;
- une mémoire des essais, erreurs et corrections.

Formule :

> L’autonomie ne vaut que si elle change les capacités des joueurs légitimes, pas seulement la position des joueurs déjà installés.

---

## 15. Méthode des terrains féconds comme changement de jeu

La méthode des terrains féconds consiste à ne pas accepter passivement le terrain imposé par une opposition stérile.

Au lieu de choisir entre deux camps déjà configurés, elle demande :

- quelle vérité partielle chaque camp protège-t-il ?
- à quelle couche cette vérité devient-elle juste ?
- quelle pathologie apparaît lorsqu’elle se prend pour le tout ?
- comment reformuler le jeu pour rendre les vérités compatibles sans les dissoudre ?

Sous l’angle de la théorie des jeux, cette méthode est un changement du jeu délibératif. Elle modifie :

- les critères de victoire ;
- les gains symboliques ;
- les possibilités d’alliance ;
- les coûts de la caricature ;
- les points focaux ;
- la capacité à coopérer cognitivement.

Exemple fondateur :

> Marché ou commun ?

peut devenir :

> Protocole commun, opérations libres.

Ce déplacement n’est pas un compromis mou. C’est une reconfiguration du terrain. Il permet de protéger à la fois la liberté d’initiative et l’existence d’un protocole commun anti-capture.

---

## 16. #1755 comme stratégie de reconnaissance minimale

Le projet #1755 cherche à faire reconnaître la place de la Corse dans l’histoire mondiale de la souveraineté populaire, du constitutionnalisme et des révolutions modernes.

Sous l’angle de la théorie des jeux, il ne faut pas commencer par la revendication maximale. Il faut identifier le plus petit énoncé historiquement rigoureux, difficile à contester, capable d’ouvrir le jeu sans déclencher immédiatement une coalition de blocage.

Cette stratégie relève d’une prudence rationnelle :

- obtenir une première reconnaissance limitée ;
- réduire le coût d’acceptation pour les interlocuteurs ;
- éviter les formulations inutilement conflictuelles ;
- stabiliser un point d’appui ;
- élargir ensuite le champ des conséquences possibles.

Formule :

> Une petite reconnaissance juste peut devenir un grand déplacement stratégique si elle modifie durablement le terrain des reconnaissances futures.

---

## 17. Corpus personnel contre profiling sauvage

Le corpus personnel et le jumeau numérique peuvent aussi être lus comme une réponse stratégique à un jeu déjà en cours.

Aujourd’hui, de nombreuses plateformes produisent des profils implicites : elles infèrent des préférences, des vulnérabilités, des comportements, des relations, des intérêts et des probabilités d’action. Ce profiling est souvent opaque, dispersé, non gouverné par la personne concernée et orienté vers des intérêts externes.

Le corpus personnel inverse cette logique :

- la personne reprend l’initiative de la description d’elle-même ;
- les sources sont versionnées ;
- les interprétations restent contestables ;
- les agents IA doivent s’aligner sur un corpus explicite plutôt que sur un profil sauvage ;
- le jumeau numérique devient une approximation gouvernée, non une capture commerciale.

Sous l’angle de la théorie des jeux, il s’agit de reprendre le contrôle du modèle de soi. Celui qui contrôle le modèle contrôle en partie les prédictions, les suggestions, les accès, les opportunités et les récits.

Formule :

> Le corpus personnel transforme un jeu où les plateformes modélisent les personnes sans mandat en un jeu où la personne rend son propre modèle explicite, gouverné, transmissible et contestable.

---

## 18. Risques et objections

### 18.1. Risque de tout transformer en jeu

Objection : cette grille pourrait réduire la vie sociale à des calculs stratégiques.

Réponse : le but n’est pas de nier l’amour, la loyauté, la dignité, la mémoire, le deuil, la beauté ou la gratuité. Le but est de comprendre pourquoi ces valeurs sont souvent détruites lorsque les règles sociales récompensent l’anti-jeu.

### 18.2. Risque de cynisme

Objection : parler de stratégie, de triche, de capture et d’incitations peut rendre cynique.

Réponse : le cynisme consiste à croire que rien ne peut changer. La théorie des jeux, utilisée correctement, permet au contraire d’identifier ce qu’il faut modifier pour que d’autres comportements deviennent rationnels.

### 18.3. Risque de surveillance

Objection : la traçabilité peut devenir surveillance.

Réponse : oui. C’est pourquoi le corpus doit maintenir une distinction stricte entre actes engageants et vie privée, entre mandataires et personnes ordinaires, entre transparence démocratique et exposition personnelle.

### 18.4. Risque de réputation punitive

Objection : Kudos, la trace ou la mémoire publique pourraient dériver vers une notation sociale.

Réponse : ce risque doit être traité dès la conception. Une réputation légitime doit être volontaire, contextualisée, reliée à des actes vérifiables, contestable, corrigeable et limitée. Elle ne doit pas devenir une note globale de valeur personnelle.

### 18.5. Risque de technosolutionnisme

Objection : les outils techniques ne suffiront pas.

Réponse : exact. Les outils ne suffisent pas. Mais sans outils de mémoire, de vérification, de coordination et de correction, les appels à la vertu restent souvent désarmés face aux jeux capturés.

### 18.6. Risque de capture du dispositif anti-capture

Objection : tout dispositif anti-capture peut être capturé à son tour.

Réponse : c’est une objection forte. Elle impose modularité, redondance, auditabilité, sortie possible, pluralité d’opérateurs, gouvernance ouverte, contrôle humain et capacité de fork.

---

## 19. Conséquences opérationnelles

Cette lecture par la théorie des jeux conduit à plusieurs exigences pratiques.

### 19.1. Pour chaque projet

Demander :

1. Quels sont les joueurs ?
2. Quels sont les coups possibles ?
3. Qui contrôle les règles ?
4. Qu’est-ce qui est visible ?
5. Qu’est-ce qui reste opaque ?
6. Qui gagne si rien ne change ?
7. Qui perd si la coopération augmente ?
8. Comment la triche est-elle détectée ?
9. Comment la coopération est-elle récompensée ?
10. Comment éviter que le remède soit capturé ?

### 19.2. Pour chaque document source

Ajouter si utile une section :

```markdown
## Lecture théorie des jeux

- Joueurs : ...
- Règles actuelles : ...
- Incitations actuelles : ...
- Asymétries d’information : ...
- Formes de triche ou d’anti-jeu : ...
- Risques de capture du control plane : ...
- Changement de jeu proposé : ...
- Mécanismes de coopération : ...
- Mécanismes de correction : ...
- Risques résiduels : ...
```

### 19.3. Pour les produits publics

La version publique peut partir de formulations simples :

- « Les règles du jeu sont cassées. »
- « Quand les tricheurs gagnent, il faut changer les règles. »
- « Acheter le match n’est pas jouer mieux. »
- « Pas de pouvoir sans contrôle. »
- « Rendre capable, c’est changer les coups possibles. »
- « L’autonomie de papier change les titres ; l’autonomie de capacité change le jeu. »

---

## 20. Prompt de rédacteur

Ce prompt sert à demander à un agent IA de produire une version, une section ou une déclinaison du présent document dans le rôle de **rédacteur**. Le rédacteur n’est pas souverain : il propose une formulation structurée, mais l’arbitrage doctrinal appartient à l’auteur humain.

```markdown
Tu es rédacteur de corpus pour Jean Hugues Noël Robert.

Mission : produire un document source autoporteur, précis, structuré et révisable, à partir des orientations fournies.

Sujet : théorie des jeux comme grille de lecture transversale du corpus.

Contraintes de fond :
- Ne pas supposer que le lecteur connaît le corpus.
- Introduire chaque notion spécifique avant de l’utiliser : C.O.R.S.I.C.A., Institut Mariani, Cogentia, Kudocracy, Kudos, FractaVolta, Fractanet, Operium, Actarchia, Mare Nostrum, Autonomie de Capacité, #1755.
- Distinguer clairement les notions générales bien connues de théorie des jeux et les formulations propres au corpus.
- Ne pas transformer la théorie des jeux en cynisme anthropologique.
- Montrer que l’enjeu est de rendre la coopération rationnelle, la triche coûteuse, les actes engageants traçables et la capture plus difficile.
- Maintenir la frontière démocratique : les agents IA suggèrent et éclairent, mais les personnes vivantes gouvernent.
- Préserver la distinction entre transparence des actes publics / mandataires et protection de la vie privée des personnes.
- Utiliser la distinction control plane / data plane pour analyser la capture.
- Prévoir les objections fortes : surveillance, crédit social, technosolutionnisme, manipulation, réduction du politique au calcul.

Style :
- Français clair, dense, non jargonnant.
- Document source GitHub, pas billet d’humeur.
- Sections numérotées.
- Définitions explicites.
- Formules doctrinales courtes lorsque stabilisées.
- Aucune référence interne non expliquée.

Format attendu :
1. Frontmatter YAML.
2. Statut du document.
3. Résumé.
4. Vocabulaire minimal de théorie des jeux.
5. Présentation autoporteuse des projets du corpus.
6. Thèse centrale.
7. Développement par couches.
8. Objections et risques.
9. Conséquences opérationnelles.
10. Prompts rédacteur et reviewer.
11. Changelog.

Rappel : tu rédiges, tu ne décides pas. Les formulations doctrinales finales doivent rester arbitrables par Jean Hugues Noël Robert.
```

---

## 21. Prompt de reviewer

Ce prompt sert à faire relire le document par un agent ou un humain dans le rôle de **reviewer critique adverse**. Le reviewer ne réécrit pas tout le texte. Il identifie les objections, les risques, les faiblesses et les corrections actionnables.

```markdown
Tu es reviewer critique adverse, non décisionnel.

Document cible :
- repository: JeanHuguesRobert/barons-Mariani
- file: research/theorie_des_jeux_corpus.md
- version: v0.1

Objectif : relire le document comme un lecteur exigeant qui ne connaît pas nécessairement le corpus de Jean Hugues Noël Robert.

Vérifie en priorité :
1. Autoportance : les notions internes sont-elles introduites avant usage ?
2. Clarté : le lecteur comprend-il la thèse sans connaître les autres documents ?
3. Théorie des jeux : les notions générales sont-elles utilisées correctement, sans surpromesse académique ?
4. Risque de réductionnisme : le texte donne-t-il l’impression de réduire l’humain au calcul stratégique ?
5. Risque politique : certaines formulations peuvent-elles être lues comme cyniques, manipulatoires ou paranoïaques ?
6. Risque démocratique : la traçabilité est-elle suffisamment distinguée de la surveillance ?
7. Risque réputationnel : Kudos ou la mémoire des actes peuvent-ils être confondus avec un crédit social ?
8. Risque IA : la frontière entre suggestion IA et prescription IA est-elle assez nette ?
9. Structure : certaines sections doivent-elles être déplacées, resserrées ou fusionnées ?
10. Produits déclinés : quelles sections sont adaptées à GitHub, Substack, Facebook ou discours politique ?

Format de réponse attendu :

## 1. Synthèse de la thèse
Résume loyalement la thèse en 5 à 10 lignes.

## 2. Points forts
Liste les éléments robustes.

## 3. Objections bloquantes
Table : objection / gravité / passage concerné / correction proposée.

## 4. Objections fortes mais non bloquantes
Table : objection / gravité / passage concerné / correction proposée.

## 5. Concepts à stabiliser
Liste les notions trop floues ou trop propres au corpus.

## 6. Risques de mauvaise lecture publique
Indique les passages pouvant être caricaturés.

## 7. Améliorations actionnables
Propose des modifications concrètes, sans réécrire tout le document.

## 8. Verdict
Choisir un statut :
- draft fragile
- draft solide mais à corriger
- release candidate après corrections mineures
- publiable comme source de travail

Rappel : tu n’es pas l’auteur. Tu aides à rendre les objections visibles et les décisions traçables. L’arbitrage final appartient à Jean Hugues Noël Robert.
```

---

## 22. Changelog

### v0.1 — 2026-06-25

Première version source.

Ajouts principaux :

- lecture transversale du corpus sous l’angle de la théorie des jeux ;
- vocabulaire minimal autoporteur ;
- introduction des notions internes du corpus ;
- articulation prime au vice / punition de la vertu ;
- distinction control plane / data plane ;
- application à Kudocracy, Kudos, FractaVolta, Cogentia, Operium, Actarchia, Autonomie de Capacité, #1755 et corpus personnel ;
- intégration de la pédagogie football / échecs ;
- objections principales ;
- prompt de rédacteur ;
- prompt de reviewer.
