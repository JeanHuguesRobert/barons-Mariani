...
The distinction between possibility and potentiality developed in §2 remains the operative one; **The Possible** sits a level above it, as the domain within which possibilities and potentialities are situated:

```text
possibilities = situated visible options
the possible  = abstract domain of what can be
The Possible  = doctrinal term for the full domain of what reality permits
...
### 1.4 Maps, Territory, and modeled possibility spaces

Rational exploration requires representations, but Potentics MUST distinguish those representations from what they attempt to describe.

**The Possible** is the wider Reality-constrained domain. Its complete boundary is not assumed to be known in advance.

A **Map** is a corrigible representation used to orient exploration. A represented candidate is therefore **not thereby established as possible**: a Map may contain unsupported hypotheses, false routes, inconsistent states or candidates that later encounters with Reality exclude.

A **Possibility Space** is an explicit model that gives coordinates, states, relations or dimensions to a family of represented candidate possibles. It is therefore a kind of Map, not The Possible itself, and may contain candidates that later prove impossible.

**Territory** is the operational metaphor for the portion or aspect of the Real actually encountered by exploration and capable of resisting, enabling, surprising or redirecting the explorer. Territory need not be geographical; a technical, legal, institutional, biological, formal or social system may constitute the encountered Territory of an inquiry.

The minimal epistemic loop is:

```text
Map_n
→ candidate Path / hypothesis
→ bounded act or observation
→ Territory
→ Reality response
→ trace + residue
→ correction
...
A **Path** is candidate or planned; a **Trajectory** is historical — what was actually attempted or traversed.

Potentics may use **Fields** to assign structured quantities to explicitly declared domains, and **Landscapes** to project one or more such fields into navigable representations. These are modeling instruments, not claims that every Potentic quantity shares one mathematical space or that The Possible is itself a known field.

The detailed exploration vocabulary, including distributed maps, Vehicles, Cognitive Packets, Attractors, Informational Gravity, Ithaca and Reactive Corpus projections, is maintained in [`research/potentics_exploration_ontology.md`](potentics_exploration_ontology.md). Those operational projections are deliberately kept outside the core definition so that Potentics does not depend on one computational architecture.

> **The map guides exploration. The territory answers.**
