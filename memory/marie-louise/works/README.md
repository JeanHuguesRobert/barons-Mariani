---
title: "Marie-Louise — créations et traces artistiques"
author: "Jean Hugues Noël Robert"
date: "2026-09-08"
document_role: "source-index"
document_kind: "memory"
visibility: "public"
lifecycle_state: "working"
update_policy: "UP-DEFAULT-REVIEWED"
review:
  status: "unreviewed"
  reviewed_by: []
---

# Marie-Louise — créations et traces artistiques

## Objet

Ce dossier rassemble des créations directes de Marie-Louise et des traces contemporaines de son parcours artistique.

Il applique la discipline documentaire de `memory/marie-louise/carte.md` :

- `VOICE` : parole ou œuvre directement attribuable à Marie-Louise ;
- `TRACE` : pièce primaire contemporaine ;
- `THIRD-PARTY` : source tierce contemporaine ;
- `FACT` : fait suffisamment établi ;
- `INFERENCE` : interprétation ;
- `UNKNOWN` : lacune ou point non résolu.

## Notes

- [`le_dieu_de_leau_2008.md`](le_dieu_de_leau_2008.md) — œuvre d'enfance directement conservée par un mail original du 3 décembre 2008.
- [`frontieres_stop_motion_2016.md`](frontieres_stop_motion_2016.md) — exposition autour des frontières à Corte, stop motion, « fragmentation de l'être », orientation artistique et candidature aux Beaux-Arts de Paris.

## Sources verbatim publiées

Pour *Le dieu de l'eau*, Jean Hugues Noël Robert a explicitement autorisé le 8 septembre 2026 la publication publique de l'œuvre intégrale et du mail original afin qu'ils constituent des sources primaires pour le futur *Suicide Corse*.

- [texte intégral verbatim](sources/le_dieu_de_leau_2008_verbatim.txt)
- [mail original RFC822](sources/2008-12-03_jouyeux-Noel-de-lilou_original.eml)
- [empreintes SHA-256](sources/SHA256SUMS.txt)

Cette autorisation est spécifique à ces deux artefacts. Elle ne vaut pas autorisation générale de publier d'autres correspondances privées de Marie-Louise ou de la famille.

## Règle de publication

Par défaut, une œuvre ou une correspondance privée conservée dans Gmail n'est pas automatiquement destinée à être reproduite intégralement dans un dépôt public.

Toute publication intégrale doit donc reposer sur une décision explicite, tracée et limitée au matériau concerné.
